        ...
        if sum(units_idx.values) >= min_n_cell:
            ...
            for train_ind,test_ind in kcv.split(spikes_zsc_ns):
                ...
                for aaa in np.arange(X_fw_ns.shape[1]):
                    for X in X_:
                        if str(X) == 'X_fw_ns':
                            ...
                            ncPCA_fw_ns_train = np.dot(train_ns,X[:,aaa])
                            ncPCA_fw_sg_train = np.dot(train_sg,X[:,aaa])
                            ncPCA_fw_ns_test  = np.dot(test_ns,X[:,aaa])
                            ncPCA_fw_sg_test  = np.dot(test_sg,X[:,aaa])
                ...
                for alpha in np.arange(len(alpha_values)):
                    ...
                    cPCs = cPCA(train_sg,train_ns)[:,:n_components]
                    
                    for aaa in np.arange(cPCs.shape[1]):
                        #projection data sets into the proper cPCs
                        cPCA_fw_ns_train = np.dot(train_ns,cPCs[:,aaa])
                        cPCA_fw_sg_train = np.dot(train_sg,cPCs[:,aaa])
                        cPCA_fw_ns_test  = np.dot(test_ns,cPCs[:,aaa])
                        cPCA_fw_sg_test  = np.dot(test_sg,cPCs[:,aaa])
                ...
                for aaa in np.arange(Vns.shape[1]):
                    ...
                    PCA_ns_train = np.dot(train_ns,Vns[:,aaa])
                    PCA_ns_test = np.dot(test_ns,Vns[:,aaa])
                    PCA_sg_train = np.dot(train_sg,Vsg[:,aaa])
                    PCA_sg_test = np.dot(test_sg,Vsg[:,aaa])
            ...
            #saving ncPCA        
            brain_area_dict['scores_ncPCA_fw_ns_' + array_of_ba[aa]] = scores_ncPCA_fw_ns
            brain_area_dict['scores_ncPCA_fw_sg_'+array_of_ba[aa]] = scores_ncPCA_fw_sg
            brain_area_dict['scores_ncPCA_bw_ns_'+array_of_ba[aa]] = scores_ncPCA_bw_ns
            brain_area_dict['scores_ncPCA_bw_sg_'+array_of_ba[aa]] = scores_ncPCA_bw_sg
            
            #saving cPCA
            brain_area_dict['scores_cPCA_fw_ns_' + array_of_ba[aa]] = scores_cPCA_fw_ns
            brain_area_dict['scores_cPCA_fw_sg_'+array_of_ba[aa]] = scores_cPCA_fw_sg
            brain_area_dict['scores_cPCA_bw_ns_'+array_of_ba[aa]] = scores_cPCA_bw_ns
            brain_area_dict['scores_cPCA_bw_sg_'+array_of_ba[aa]] = scores_cPCA_bw_sg
            
            #saving PCA
            brain_area_dict['scores_PCA_ns_' + array_of_ba[aa]] = scores_PCA_ns
            brain_area_dict['scores_PCA_sg_'+array_of_ba[aa]] = scores_PCA_sg
